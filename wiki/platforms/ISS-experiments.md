---
title: "ISS experiments (µg lab)"
category: platform
tags: [platform, ISS, microgravity, facility]
papers:
  - "[[../Benchmark Al-Cu Solidification Experiments in Microgravity and on Earth_THOMAS.pdf]]"
  - "[[../Structures in grain-refined directionally solidified hypoeutectic Al-Cu alloys- Benchmark experiments under microgravity on-board the International Space Station_ZIMMERMANN.pdf]]"
  - "[[../In situ investigation of the Columnar-to-Equiaxed Transition during microgravity recoral solidification of Al-20 wt cu alloys on Earth_NGOMESSE.pdf]]"
  - "[[../Analysis of gravity effects during binary alloy directional solidification by comparison of microgravity and Earth experiments with in situ observation_BERGEON.pdf]]"
  - "[[../Nucleation and Growth Dynamics of Equiaxed Dendrites in Thin Metallic Al-Cu and Al-Ge Samples in Microgravity and on Earth_BECKER.pdf]]"
  - "[[../Impact of gravity on directional solidification of refined Al-20wt.%Cu alloy investigated by in situ X-radiography_HARJER.pdf]]"
  - "[[../Analysis of preparatory directional solidification experiments for a new X-ray facility for the International Space Station_REINHART.pdf]]"
  - "[[../In-situ X-ray monitoring of solidification and related processes of metal alloys_REINHART.pdf]]"
  - "[[../Spurious grain formation due to Marangoni convection during directional solidification of alloys in u-g environment of International Space Station_SEYED.pdf]]"
  - "[[../Microgravity studies of solidification patterns in model transparent alloys onboard the International Space Station_AKAMATSU.pdf]]"
  - "[[../Demonstration of the effect of stirring on nucleation from experiments on the International Space Station using the ISS-EML facility_GANGOPADHYAY.pdf]]"
  - "[[../Electromagnetic levitation containerless processing of metallic materials in microgravity- rapid solidification_MATSON.pdf]]"
  - "[[../Electromagnetic levitation containerless processing of metallic materials in microgravity- thermophysical properties_MOHR.pdf]]"
  - "[[../Precise Measurements of Thermophysical Properties of Liquid Ti-6AI-4V (Ti64) Alloy On Board the International Space Station_MOHR.pdf]]"
  - "[[../The Effect of g-Level Fluctuations on Equiaxed Solidification of an Al-20wt.%Cu Alloy in Microgravity_GENG.pdf]]"
related:
  - "[[wiki/platforms/CETSOL-MICAST-ICESOL]]"
  - "[[wiki/platforms/DECLIC-DSI]]"
  - "[[wiki/platforms/sounding-rockets]]"
  - "[[wiki/platforms/containerless-EML]]"
  - "[[wiki/platforms/ISM-strategy]]"
  - "[[wiki/platforms/commercial-ISM-Varda]]"
  - "[[wiki/physics/solidification]]"
  - "[[wiki/physics/nucleation-CET]]"
  - "[[wiki/physics/freckle-segregation]]"
  - "[[wiki/physics/mushy-zone]]"
  - "[[wiki/physics/dendritic-growth]]"
  - "[[wiki/physics/eutectic-solidification]]"
  - "[[wiki/flow/marangoni-flow]]"
  - "[[wiki/flow/residual-gravity-gjitter]]"
  - "[[wiki/methods/synchrotron-Xray]]"
  - "[[wiki/methods/directional-solidification]]"
  - "[[wiki/methods/EBSD-characterisation]]"
  - "[[wiki/alloys/Al-Cu]]"
  - "[[wiki/alloys/Al-Si]]"
  - "[[wiki/alloys/Ti-alloys]]"
  - "[[wiki/alloys/SCN-transparent-analog]]"
  - "[[wiki/alloys/Ni-superalloys]]"
created: 2026-04-18
---

# ISS experiments (µg lab)

**Category:** platform

---

## Overview

The ISS hosts the most capable µg metal-solidification laboratory ever
flown: long-duration near-diffusive conditions for directional
solidification, plus ISS-EML and ESL containerless processing. Key
facilities: **SQF / LGF** furnaces in the MSL (NASA ESA Materials
Science Laboratory), **ISS-EML** (Columbus module), **MSG / MSRR**, and
the upcoming **XRMON-ISS** X-ray facility.

## Residual environment

Quasi-steady **g₀ ~ 10⁻⁶ g** plus [[wiki/flow/residual-gravity-gjitter|g-jitter]] from
crew and machinery. Marangoni flow is *not* suppressed, and spurious
grain formation has been documented ([[../Spurious grain formation due to Marangoni convection during directional solidification of alloys in u-g environment of International Space Station_SEYED.pdf|Seyed — Marangoni-driven spurious grains ISS]]).

## Campaigns represented in this wiki

- [[wiki/platforms/CETSOL-MICAST-ICESOL|CETSOL MICAST ICESOL]] / MICAST: [[../Benchmark Al-Cu Solidification Experiments in Microgravity and on Earth_THOMAS.pdf|Thomas 2023 — Al-Cu benchmark µg+1g]],
  [[../Structures in grain-refined directionally solidified hypoeutectic Al-Cu alloys- Benchmark experiments under microgravity on-board the International Space Station_ZIMMERMANN.pdf|Zimmermann — grain-refined Al-Cu ISS]], [[../In situ investigation of the Columnar-to-Equiaxed Transition during microgravity recoral solidification of Al-20 wt cu alloys on Earth_NGOMESSE.pdf|Ngomesse 2023 — Al-20Cu CET in-situ]],
  [[../Analysis of gravity effects during binary alloy directional solidification by comparison of microgravity and Earth experiments with in situ observation_BERGEON.pdf|Bergeon 2022 — µg vs 1g DS in-situ]], [[../Impact of gravity on directional solidification of refined Al-20wt.%Cu alloy investigated by in situ X-radiography_HARJER.pdf|Härjer — Al-20Cu in-situ X-radiography]].
- [[wiki/platforms/DECLIC-DSI|DECLIC DSI]]: [[../Microgravity studies of solidification patterns in model transparent alloys onboard the International Space Station_AKAMATSU.pdf|Akamatsu — SCN transparent µg]].
- ISS-EML: [[../Electromagnetic levitation containerless processing of metallic materials in microgravity- thermophysical properties_MOHR.pdf|Mohr — ISS-EML thermophysical properties]], [[../Electromagnetic levitation containerless processing of metallic materials in microgravity- rapid solidification_MATSON.pdf|Matson — ISS-EML rapid solidification]],
  [[../Precise Measurements of Thermophysical Properties of Liquid Ti-6AI-4V (Ti64) Alloy On Board the International Space Station_MOHR.pdf|Mohr — Ti-6Al-4V ISS-EML]], [[../Demonstration of the effect of stirring on nucleation from experiments on the International Space Station using the ISS-EML facility_GANGOPADHYAY.pdf|Gangopadhyay 2022 — ISS-EML stirring & nucleation]].
- XRMON prep: [[../Analysis of preparatory directional solidification experiments for a new X-ray facility for the International Space Station_REINHART.pdf|Reinhart 2022 — XRMON-ISS prep]].

## Connected topics

- [[wiki/platforms/CETSOL-MICAST-ICESOL|CETSOL MICAST ICESOL]] · [[wiki/platforms/DECLIC-DSI|DECLIC DSI]] · [[wiki/platforms/sounding-rockets|sounding rockets]] · [[wiki/platforms/containerless-EML|containerless EML]]
- [[wiki/physics/solidification|solidification]] · [[wiki/physics/nucleation-CET|nucleation CET]] · [[wiki/physics/freckle-segregation|freckle segregation]] · [[wiki/physics/eutectic-solidification|eutectic solidification]]
- [[wiki/flow/marangoni-flow|marangoni flow]] · [[wiki/flow/residual-gravity-gjitter|residual gravity gjitter]]
- [[wiki/methods/synchrotron-Xray|synchrotron Xray]] · [[wiki/methods/directional-solidification|directional solidification]] · [[wiki/methods/EBSD-characterisation|EBSD characterisation]]
- [[wiki/alloys/Al-Cu|Al Cu]] · [[wiki/alloys/Al-Si|Al Si]] · [[wiki/alloys/Ti-alloys|Ti alloys]] · [[wiki/alloys/SCN-transparent-analog|SCN transparent analog]] · [[wiki/alloys/Ni-superalloys|Ni superalloys]]

## Papers from `papers/` folder

| Paper | Key finding |
|-------|-------------|
| [[../Benchmark Al-Cu Solidification Experiments in Microgravity and on Earth_THOMAS.pdf|Thomas 2023 — Al-Cu benchmark µg+1g]] | Canonical Al-Cu ISS benchmark for CET models |
| [[../Structures in grain-refined directionally solidified hypoeutectic Al-Cu alloys- Benchmark experiments under microgravity on-board the International Space Station_ZIMMERMANN.pdf|Zimmermann — grain-refined Al-Cu ISS]] | ISS benchmark on grain-refined hypoeutectic Al-Cu |
| [[../In situ investigation of the Columnar-to-Equiaxed Transition during microgravity recoral solidification of Al-20 wt cu alloys on Earth_NGOMESSE.pdf|Ngomesse 2023 — Al-20Cu CET in-situ]] | In-situ CET observation in Al-20Cu, µg vs 1g |
| [[../Analysis of gravity effects during binary alloy directional solidification by comparison of microgravity and Earth experiments with in situ observation_BERGEON.pdf|Bergeon 2022 — µg vs 1g DS in-situ]] | Directly contrasts µg vs 1g directional solidification patterns |
| [[../Nucleation and Growth Dynamics of Equiaxed Dendrites in Thin Metallic Al-Cu and Al-Ge Samples in Microgravity and on Earth_BECKER.pdf|Becker 2023 — Al-Cu / Al-Ge equiaxed µg]] | Equiaxed nucleation/growth in thin Al-Cu / Al-Ge under µg |
| [[../Impact of gravity on directional solidification of refined Al-20wt.%Cu alloy investigated by in situ X-radiography_HARJER.pdf|Härjer — Al-20Cu in-situ X-radiography]] | In-situ X-radiography of grain-refined Al-20Cu DS |
| [[../Analysis of preparatory directional solidification experiments for a new X-ray facility for the International Space Station_REINHART.pdf|Reinhart 2022 — XRMON-ISS prep]] | Ground-based preparation for new ISS X-ray facility (XRMON) |
| [[../In-situ X-ray monitoring of solidification and related processes of metal alloys_REINHART.pdf|Reinhart — in-situ X-ray review]] | Review of in-situ X-ray monitoring in alloy solidification |
| [[../Spurious grain formation due to Marangoni convection during directional solidification of alloys in u-g environment of International Space Station_SEYED.pdf|Seyed — Marangoni-driven spurious grains ISS]] | Marangoni convection creates spurious grains on ISS DS |
| [[../Microgravity studies of solidification patterns in model transparent alloys onboard the International Space Station_AKAMATSU.pdf|Akamatsu — SCN transparent µg]] | ISS studies of patterns in transparent SCN-based analogs |
| [[../Demonstration of the effect of stirring on nucleation from experiments on the International Space Station using the ISS-EML facility_GANGOPADHYAY.pdf|Gangopadhyay 2022 — ISS-EML stirring & nucleation]] | Stirring modifies nucleation in ISS-EML Zr-based liquids |
| [[../Electromagnetic levitation containerless processing of metallic materials in microgravity- rapid solidification_MATSON.pdf|Matson — ISS-EML rapid solidification]] | ISS-EML rapid solidification of industrial alloys |
| [[../Electromagnetic levitation containerless processing of metallic materials in microgravity- thermophysical properties_MOHR.pdf|Mohr — ISS-EML thermophysical properties]] | Density, viscosity, surface tension via ISS-EML |
| [[../Precise Measurements of Thermophysical Properties of Liquid Ti-6AI-4V (Ti64) Alloy On Board the International Space Station_MOHR.pdf|Mohr — Ti-6Al-4V ISS-EML]] | Precise thermophysics of liquid Ti64 via ISS-EML |
| [[../The Effect of g-Level Fluctuations on Equiaxed Solidification of an Al-20wt.%Cu Alloy in Microgravity_GENG.pdf|Geng — g-jitter on Al-20Cu]] | g-jitter modifies equiaxed Al-20Cu solidification in µg |

---
*Last updated: 2026-04-18 · Auto-generated via LLM Wiki*