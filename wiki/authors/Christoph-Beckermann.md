---
title: "Christoph Beckermann"
category: author
tags: [author, macrosegregation, freckle, volume-averaging]
papers:
  - "[[../Spurious grain formation due to Marangoni convection during directional solidification of alloys in u-g environment of International Space Station_SEYED.pdf]]"
  - "[[../Benchmark Al-Cu Solidification Experiments in Microgravity and on Earth_THOMAS.pdf]]"
  - "[[../Mesoscopic modeling of equiaxed and columnar solidification microstructures under forced flow and buoyancy-driven flow in hypergravity_VIARDIN.pdf]]"
  - "[[../Mesoscale Front Tracking Simulation of a Microgravity-based Isothermal Equiaxed Solidification Experiment of an Al20wt.%Cu Alloy performed on board the MASER 13 Sounding Rocket_MURPHY.pdf]]"
related:
  - "[[wiki/physics/freckle-segregation]]"
  - "[[wiki/physics/mushy-zone]]"
  - "[[wiki/physics/nucleation-CET]]"
  - "[[wiki/flow/thermosolutal-convection]]"
  - "[[wiki/modelling/CA-FE-models]]"
  - "[[wiki/modelling/sharp-interface]]"
  - "[[wiki/modelling/phase-field]]"
  - "[[wiki/methods/CFD-simulation]]"
  - "[[wiki/modelling/dimensionless-numbers]]"
  - "[[wiki/alloys/Al-Cu]]"
  - "[[wiki/alloys/Ni-superalloys]]"
  - "[[wiki/platforms/ISS-experiments]]"
  - "[[wiki/platforms/CETSOL-MICAST-ICESOL]]"
created: 2026-04-18
---

# Christoph Beckermann

**Category:** author

---

## Overview

**Christoph Beckermann** (University of Iowa) is the originator of
**volume-averaged two-phase models** of alloy solidification, central to
predicting [[wiki/physics/freckle-segregation|freckle segregation]] defects in turbine blades. His group's
multiphase codes (MICRESS-derived, and commercial CAFÉ) are benchmarked
against the Al-Cu ISS data reviewed in [[wiki/physics/nucleation-CET|nucleation CET]].

## Why Beckermann matters here

- Volume-averaged framework → standard in [[wiki/modelling/CA-FE-models|CA-FE]] and
  [[wiki/modelling/sharp-interface|sharp-interface]] models.
- Double-diffusive freckle theory → directly relevant to
  [[../Spurious grain formation due to Marangoni convection during directional solidification of alloys in u-g environment of International Space Station_SEYED.pdf|Seyed — Marangoni-driven spurious grains ISS]] and to Ni-base single-crystal DS.
- His Al-Cu benchmark work shapes [[../Benchmark Al-Cu Solidification Experiments in Microgravity and on Earth_THOMAS.pdf|Thomas 2023 — Al-Cu benchmark µg+1g]].

## Connected topics

- [[wiki/physics/freckle-segregation|freckle segregation]] · [[wiki/physics/mushy-zone|mushy zone]] · [[wiki/physics/nucleation-CET|nucleation CET]] · [[wiki/flow/thermosolutal-convection|thermosolutal convection]]
- [[wiki/modelling/CA-FE-models|CA FE models]] · [[wiki/modelling/sharp-interface|sharp interface]] · [[wiki/modelling/phase-field|phase field]] · [[wiki/methods/CFD-simulation|CFD simulation]]
- [[wiki/alloys/Al-Cu|Al Cu]] · [[wiki/alloys/Ni-superalloys|Ni superalloys]]
- [[wiki/platforms/ISS-experiments|ISS experiments]] · [[wiki/platforms/CETSOL-MICAST-ICESOL|CETSOL MICAST ICESOL]]

## Papers from `papers/` folder

| Paper | Key finding |
|-------|-------------|
| [[../Spurious grain formation due to Marangoni convection during directional solidification of alloys in u-g environment of International Space Station_SEYED.pdf|Seyed — Marangoni-driven spurious grains ISS]] | Marangoni convection creates spurious grains on ISS DS |
| [[../Benchmark Al-Cu Solidification Experiments in Microgravity and on Earth_THOMAS.pdf|Thomas 2023 — Al-Cu benchmark µg+1g]] | Canonical Al-Cu ISS benchmark for CET models |
| [[../Mesoscopic modeling of equiaxed and columnar solidification microstructures under forced flow and buoyancy-driven flow in hypergravity_VIARDIN.pdf|Viardin — mesoscopic hypergravity]] | Mesoscopic CET modelling under forced+buoyant flow |
| [[../Mesoscale Front Tracking Simulation of a Microgravity-based Isothermal Equiaxed Solidification Experiment of an Al20wt.%Cu Alloy performed on board the MASER 13 Sounding Rocket_MURPHY.pdf|Murphy — MASER-13 front-tracking Al-20Cu]] | Mesoscale front-tracking of MASER-13 Al-20Cu experiment |

---
*Last updated: 2026-04-18 · Auto-generated via LLM Wiki*